...
class questionGeneration(Action):
    ...
    async def run(self, knowledge_description: str):
        qtype = [
            ...
            {
                "名称": "症状诊断题",
                "特点": "通过患者的症状、体征和实验室检查结果，采用通俗化的语言，模拟病例或描绘场景，确保描述能引导考生通过归纳和提取信息来作答，考察考生的临床推理能力。",
                "输出案例": [
                    {
            ...
            {
                "名称": "题干分析题",
                "特点": "给定一个医学场景或背景，要求考生基于题干内容提出多个问题，考察其推理、分析与综合能力。同时通过在题干描述中加入与诊断不完全相关的症状和检查结果，考察考生从混杂信息中筛选关键信息、排除干扰项并作出正确诊断的能力。",
                "输出案例": [
                    {
